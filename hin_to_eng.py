...
def _setup():
    ...
    SCHEMES.update({
        ...
        DEVANAGARI: Scheme({
            'vowels': s("""अ आ इ ई उ ऊ ऋ ॠ ऌ ॡ ए ऐ ओ औ ऍ"""),
            'marks': s("""ा ि ी ु ू ृ ॄ ॢ ॣ े ै ो ौ ॉ"""),
            'virama': s('्'),
            'other': s('ं ः ँ'),
            # 'क़', 'ॉ', 'ज़', 'ख़', 'ऍ', 'ग़', 'ड़', 'फ़', 'ढ़'
            'consonants': s("""
        ...
        HK: Scheme({
            'vowels': s("""a A i I u U R RR lR lRR e ai o au ae"""),
            'marks': s("""A i I u U R RR lR lRR e ai o au n"""),
            'virama': [''],
            'other': s('n H ~'),
            # 'k', 'o.', 'j', 'kh', 'aa', 'g', 'd', 'f', 'dh'
            'consonants': s("""
                            k kh g gh G
                            c ch j jh J
                            T Th D Dh N
...
def find_unique_words(df_main):
    ...
    words = []
    for i in df_main.columns: #iterating on columns
        i = str(i)
...
def hin_to_eng(df):
    ...
    with open('hi_to_eng.json', 'r') as file:
        diss = json.load(file)
    converted_words = []
    for i in l:
        l2 = []
        for j in i.split(' '):
            try:
                ...
                val1 = transliterate(j, _from='devanagari', _to='hk')
            l2.append(val1)
        val2 = ' '.join(l2)
        converted_words.append(val2.lower().strip())
    
    converted_words_dict = dict(zip(unique_words, converted_words))
    
    for i in tqdm(df_main.columns):
        df[str(i) + '_converted'] = df[str(i)].apply(lambda x:np.nan if(x!=x) else ' '.join([converted_words_dict[i] for i in str(x).lower().strip().split(' ')]))
